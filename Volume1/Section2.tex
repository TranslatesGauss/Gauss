...
\subsection*{Art. 13\\ \begin{small}\textit{(Preliminary theorems about prime numbers, factors, etc.)} \end{small} }
...
\subsection*{Art. 26\\ \begin{small}\textit{(Solution of congruences of the first degree})\end{small}}
...
In the same way that the root of the equation $ax = b$ is expressed by $\frac{b}{a}$, we can also denote the root of the congruence $ax \equiv b$ by $\frac{b}{a}$, including the modulus of the congruence for the sake of clarity.  So e.g. $\frac{19}{17} \; (\textrm{mod. } 12)$ denotes any number that is $\equiv 11 \;(\textrm{mod. }12)$.\footnote{This could also analogously be denoted by $\frac{11}{1} \;(\textrm{mod. }12)$}  It is generally 
clear from the above that $\frac{b}{a} \;(\textrm{mod. }c)$ does not have any real significance (or if one prefers, it is imaginary), when $a$ and $c$ have a common divisor which does not divide $b$.  But except in this case, the expression $\frac{b}{a}\;(\textrm{mod. }c)$ will always have real values, and indeed infinitely many, but these will all be congruent modulo $c$ when $a$ is prime to $c$, or modulo $\frac{c}{\delta}$, where $\delta$ is the greatest common divisor of $c$ and $a$.
...
\item $\frac{a k}{bk}\;(\textrm{mod. }c)$ and $\frac{a}{b} \;(\textrm{mod. }c)$ are equivalent if $k$ is prime to $c$.
\end{enumerate}
...
\subsection*{Art. 32 \\ \begin{small} \textit{(On finding numbers with given remainders according to given moduli)}  \end{small}}
...
\subsection*{Art 37. \\ \begin{small} (\textit{Linear congruences involving multiple unknowns}) \end{small}}
...
\begin{example} The proposed congruences 
...
\subsection*{Art. 38 \\ \begin{small} (\textit{Various Theorems}) \end{small} } 
